...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(ST7920_RST_GPIO_Port, ST7920_RST_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin : ST7920_CS_Pin */
  ...
  /*Configure GPIO pin : ST7920_RST_Pin */
  GPIO_InitStruct.Pin = ST7920_RST_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(ST7920_RST_GPIO_Port, &GPIO_InitStruct);
...
/* ══════════════════════════════════════════════════════════════
 * sensor_task
 * Baca XY-MD02 setiap 5 detik, kirim ke queue
 * ══════════════════════════════════════════════════════════════ */
void sensor_task(void *arg)
{
    sensor_data_t data;
    ...
    while (1)
    {
        ...
        if (ret == SENSOR_OK)
        {
            /* Overwrite queue dengan data terbaru */
            osMessageQueuePut(sensor_queue, &data, 0, 0);
        }
        /* Debug via USART1 */
        char dbg[64];
                snprintf(dbg, sizeof(dbg),
                         "[SENSOR] ret=%d T=%.1f H=%.1f\r\n",
                         ret, data.temperature, data.humidity);
